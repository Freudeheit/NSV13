...
/mob/living/proc/ptt_sound_start()
	//var/list/hear_freqs = list() //if this doesnt work maybe have it be based on the person speaking in the chat?
	to_chat(world, "<span class='boldannounce'>PTT sound start called</span>")
	for(var/V in GLOB.player_list)
		var/mob/M = V
		if (hullrot_radio_allowed() && !incapacitated(ignore_grab = TRUE))
		//if(ismob(R.loc))
		//var/mob/M = usr
			M.playsound_local(M, 'sound/effects/radio2.ogg', 100, FALSE, pressure_affected = FALSE) //find a way to play sound only to client and people on radio
			to_chat(world, "<span class='boldannounce'>Sound should play</span>")

/mob/living/proc/ptt_sound_stop()
	to_chat(world, "<span class='boldannounce'>PTT sound stop called</span>")
	for(var/V in GLOB.player_list)
		var/mob/M = V
		if (hullrot_radio_allowed() && !incapacitated(ignore_grab = TRUE))
			M.playsound_local(M, 'sound/effects/radio1.ogg', 100, FALSE, pressure_affected = FALSE) //find a way to play sound only to client and people on radio
			to_chat(world, "<span class='boldannounce'>end sound should play</span>")
